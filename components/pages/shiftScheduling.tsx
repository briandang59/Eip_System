...
export default function ShiftScheduling() {
    ...
    const [search, setSearch] = useState<string>(cardNumber || '');
    ...
    // Use useRef to track initial cardNumber to avoid infinite loop
    const initialCardNumberRef = useRef(cardNumber);
    ...
    // Add useEffect to handle cardNumber changes from URL params
    useEffect(() => {
        // Only update search if cardNumber changes from URL params (not from user input)
        if (cardNumber && cardNumber !== initialCardNumberRef.current) {
            setSearch(cardNumber);
            initialCardNumberRef.current = cardNumber;
        }
    }, [cardNumber]);
